...
function build() {
    ...
    fsq.outputFile(output, header + minCode)
        .then(function() { return fsq.copy(path.join(root, 'media/'), path.join(currDir, 'build/media/')); })
        .then(function() { return fsq.readFile(currPath, 'utf-8'); })
        .then(replace)
        .then(function (data) { return fsq.writeFile(path.join(process.cwd(), 'build/index.html'), data); })
        .then(function() { return fsq.copy(path.join(__dirname, '/templates/styles.css'), path.join(currDir, 'build/styles.css')); })
        .then(function() {
