...
function build() {
    ...
    fse.outputFile(output, header + minCode, function (err) {
        ...
        fse.copy(path.join(root, 'media/'), path.join(currDir, 'build/media/'), function (err) {
            ...
            fs.readFile(currPath, 'utf-8', function (err, data) {
                ...
                fs.writeFile(path.join(process.cwd(), 'build/index.html'), data, function (err) {

                    if (err) { throw err; }

                    fse.copy(path.join(__dirname, '/templates/styles.css'), path.join(currDir, 'build/styles.css'), function (err) {
